...
from neo4jconnector import Neo4jConnector, default_config, SearchTooBroad
from utils import (
    get_trigger,
    to_options,
    get_node_ids,
...
app.neo4j = Neo4jConnector(default_config)
...
graphs = initialized_graphs(ETYMOLOGY_GRAPH, LANGUAGE_TREE, RELATIONS_TREE, dbconsts)
...
@app.callback(
    ...
    State(
        {"type": "select-button-group", "group": MATCH, "id": MATCH, "idx": ALL}, "id"
    ),
    prevent_initial_call=True,
)
...
@app.callback(
    ...
    Input(
        id_merge(
            {"type": LABEL_DROPDOWN}, aggregation.gid(aggregation.ID.SELECTED_LABELS)
        ),
        "value",
    ),
)
...
def interact_graph(
        ...
        with app.neo4j.new_session() as session:
            ...
            if CREATE_LABEL:
                if not re.fullmatch("[a-zA-z]+", new_label_text):
                    return ret(
                        status="Invalid label name", status_style=STATUS_STYLE_WARNING
                    )
    ...
    if GRAPH_TAP:
        ...
        if onclick_action == SHOW_DETAILS:
            return ret(show_details=True, details_component=render_element(event_data))
    ...
    if DO_SEARCH or not trigger_id:
        ...
        try:
            with app.neo4j.new_session() as session:
                ...
                if not simple_search_disabled or event_data and not expand_use_config:
    ...
    if "born" in event_data:
        return dbc.Col(
            [
                ...
                *([html.A(event_data["wikipedia"])] if event_data["wikipedia"] else []),
            ]
        )
        ...
        if "term" in event_data:
            ...
            if pronunciation:
                ...
                for pr in pronunciation:
                    ipa, kind, accent = [pr.get(k) for k in ["ipa", "kind", "accent"]]
                    if kind == "phonemic":
                        ipa = f"/{ipa}/"
                    elif kind == "phonetic":
                        ipa = f"[{ipa}]"
            ...
            if gloss_per_pos:
                for pos, glosses in sorted(gloss_per_pos.items()):
                    ...
                    children.append(
                        html.Ul(
                            [html.Li(text2display(gloss["text"])) for gloss in glosses]
                        )
                    )
